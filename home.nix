...
in {
  ...
  programs.zsh = {
    ...
    shellAliases = {
      ...
      # KUBE
      kubelogs =
        "kubectl get pods --all-namespaces | sed -n '1!p' | fzf --layout=reverse -m | sed 's/ .*//g' | xargs -I{} -ot kubectl logs -f {}";
      kuberm =
        "kubectl get pods --all-namespaces | sed -n '1!p' | fzf --layout=reverse -m | sed 's/ .*//g' | xargs -I{} -ot kubectl delete pod {}";
      kubesh =
        "kubectl get pods --all-namespaces | sed -n '1!p' | fzf --layout=reverse -m | sed 's/ .*//g' | xargs -I{} -ot kubectl --tty --stdin exec {} -- /bin/sh";
      # kubeinitcontext =
